...
class Notebook:
    ...
    def addTab(self, text):
        ...
        frms.append(widgetFrame)
        tbs.append(tab)
        print(self.frames, "tabs+++++++++", self.tabs)
        return widgetFrame
...
class WidgetFrame:
    ...
    def OptionMenu(self, values: list, variable: tk.Variable, command=None, args=(), default=None, row: int = None,
        ...
        if default is None:
            default = values[0]
        widget = ttk.OptionMenu(self.master, variable, default, command=partial(command, *args), *values,
                                **widgetkwargs)
        widget.grid(row=row, column=col, padx=padx, pady=pady, sticky=sticky, rowspan=rowspan, columnspan=colspan,
                    **gridkwargs)
        name = "OptionMenu"
    ...
    def Notebook(self, name, row: int = None, col: int = None, padx=10, pady=10, sticky="nsew", rowspan: int = 1,
        ...
        print(widget.frames, "tabs+++++++++", widget.tabs)
        return widget
